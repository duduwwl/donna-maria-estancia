(() => {
  ...
  function showProduct(id) {
    ...
    $('#sizeOptions').innerHTML = (quickProduct.sizes || []).map(value => `<button type="button" class="size-option" data-size="${text(value)}" aria-pressed="false">${text(value)}</button>`).join('') || '<span class="variant-confirmation">Tamanhos disponíveis: confirme com a loja.</span>';
    ...
    $('#sizeField').hidden = !(quickProduct.sizes || []).length;
    $('#colorField').hidden = !(quickProduct.colors || []).length;
    $('#detailSizeGuide').hidden = !(quickProduct.sizes || []).length;
